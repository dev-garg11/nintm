...
export default function AdminPage() {
    ...
    const fetchRegistrations = () => {
        setLoading(true);
        let query = `/api/admin?search=${searchTerm}&state=${stateFilter}&status=${statusFilter}&paymentStatus=${paymentStatusFilter}`;
        fetch(query)
            .then((res) => res.json())
            .then((data) => {
                if (data.success) {
    ...
    const handleExportCSV = () => {
        ...
        const rows = registrations.map(r => [
            ...
            r.city || '',
            r.pincode || '',
            r.fullLengthPhoto ? (window.location.origin + r.fullLengthPhoto) : '',
            r.closeUpPhoto ? (window.location.origin + r.closeUpPhoto) : '',
            r.paymentStatus || '',
            r.paymentAmount || 0,
            r.razorpayPaymentId || '',
            r.createdAt || ''
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    {selectedCandidate.fullLengthPhoto ? (
                                                        <a href={selectedCandidate.fullLengthPhoto} target="_blank" rel="noopener noreferrer" className="block relative aspect-[3/4] w-full border border-[#D4AF37]/25 hover:border-[#D4AF37] transition-all overflow-hidden bg-black/40">
                                                            <Image src={selectedCandidate.fullLengthPhoto} alt="Full Length Upload" fill className="object-contain" />
                                                ...
                                                <div>
                                                    ...
                                                    {selectedCandidate.closeUpPhoto ? (
                                                        <a href={selectedCandidate.closeUpPhoto} target="_blank" rel="noopener noreferrer" className="block relative aspect-[3/4] w-full border border-[#D4AF37]/25 hover:border-[#D4AF37] transition-all overflow-hidden bg-black/40">
                                                            <Image src={selectedCandidate.closeUpPhoto} alt="Close-Up Upload" fill className="object-contain" />
                                                            <div className="absolute right-1 bottom-1 p-0.5 bg-black/70 text-white rounded"><ExternalLink className="w-2.5 h-2.5" /></div>
                                                        </a>
                                                    ) : (
                                                        <div className="aspect-[3/4] bg-black/10 border border-white/5 flex items-center justify-center text-[9px] text-[#D9E1EC]/40 italic">Not available</div>
