...
export async function POST(request) {
    try {
        ...
        if (!paymentEntity) {
            return NextResponse.json({ success: true, message: 'Invalid payload entity, skipped.' });
        }
        ...
        if (!orderId) {
            return NextResponse.json({ success: true, message: 'No Order ID linked in entity, skipped.' });
        }
        ...
        if (!candidate) {
            ...
            return NextResponse.json({ success: true, message: 'No matching candidate registration found.' });
        }
        ...
        return NextResponse.json({ error: 'An error occurred during webhook execution.' }, { status: 500 });
    }
}
